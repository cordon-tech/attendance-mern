...
exports.createAttendance = async (req, res) => {
  ...
  try {
    ...
    const result = await mongoose.connection.db.collection('ams').updateOne(
      { _id: new mongoose.Types.ObjectId(process.env._id) }, // Replace with your actual MongoDB document ID
      { $set: { [`attendanceMaster.${currentDate}.${workerId}`]: attendanceData } },
      { upsert: true }
    );
